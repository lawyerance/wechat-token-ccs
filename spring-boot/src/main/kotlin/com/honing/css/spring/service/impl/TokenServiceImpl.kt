...
@Service
open class TokenServiceImpl : TokenService {
  private var logger: Logger = LoggerFactory.getLogger(ClientHttpRequestLoggingInterceptor::class.java)

  private lateinit var restTemplate: RestTemplate
  private lateinit var wechatOpenApi: WechatOpenApi

  fun TokenServiceImpl(restTemplate: RestTemplate, wechatOpenApi: WechatOpenApi) {
  ...
  override fun getToken(grantType: String?, appid: String?, secret: String?): Map<String, Any> {
    ...
    logger.info(
      "Cache is invalid, will using appid = {} to request -token url:  {}", appid, wechatOpenApi.accessToken
    )
